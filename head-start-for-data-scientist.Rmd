...
## rbind():
Esta función combina un vector, una matriz o un marco de datos por filas.

Agrega la columna Survived en el dataSet test, para unir los dataset train y test. 
...
## ifelse(is.na(Age)): Crea un vector FALSE Y TRUE, donde aparece un valor TRUE es un valor perdido, si es perdido calcula la media del campo Age, de otra forma deja el valor actual en Age.

## mean(full$Age, na.rm=TRUE): Removemos todos los valores perdidos del campo Age y luego saca la media y la almacena en el campo Age

## Crea un campo llamado "Age Group" Comparando el valor Age usando la funcion case_when() y los clasifica la edad en cuatro grupos Age.0012, Age1317, Age1859, Age60Ov
...
## Si hay un valor perdido en el campo Embarked, se le asinga una "S", que embarco en Southampton
## El valor es reemplazado en el campo del dataset full
...
## Dependiendo del número de miembros se agrupan por familias, Single, Small, Big.
...
## Imprimir reglas con la función rpart.rules
...
## Vamos a evaluar la capacidad del modelos con los datos no vistos
